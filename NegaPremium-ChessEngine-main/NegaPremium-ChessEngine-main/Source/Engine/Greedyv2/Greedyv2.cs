...
namespace NegaPremium {
    ...
    public sealed partial class Engine : IPlayer {
        ...
        private Int32 Greedyv2Search(Position position, Int32 depth, Int32 alpha, Int32 beta, Int32 ply, Boolean inCheck, Boolean allowNull) {
            ...
            WriteGreedyv2Statistics(position, _stopwatch.Elapsed.TotalMilliseconds);
        ...
        private void WriteGreedyv2Statistics(Position position, Double elapsed) {
            if (Restrictions.Output != OutputType.GUI)
                return;
            ...
            Terminal.WriteLine(position.ToString(
                ...
                String.Format("CNN top-N          {0}", Greedyv2TopN),
                String.Format("Static evaluation  {0:+0.00;-0.00}", Evaluator.Evaluate(position) / 100.0)));
            Terminal.WriteLine("Greedyv2 final evaluation: {0:+0.00;-0.00}", Evaluator.Evaluate(position) / 100.0);
            Terminal.WriteLine();
        }
    }
}
