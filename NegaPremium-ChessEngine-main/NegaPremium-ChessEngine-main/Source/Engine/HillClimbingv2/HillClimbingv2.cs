...
namespace NegaPremium {
    ...
    public sealed partial class Engine : IPlayer {
        ...
        private const Int32 HillClimbingv2TopN = 10;
        private const Int32 HillClimbingv2MateScore = CheckmateValue;
        private const Int32 HillClimbingv2LateMoveReductionDepth = 3;
        private const Int32 HillClimbingv2LateMoveReductionMove = 3;
        private const Int32 HillClimbingv2NullMoveReduction = 2;
        private static readonly String HillClimbingv2ModelPath = Path.Combine("ml", "models", "chess_move_matrix_cnn", "model.onnx");
        private static readonly String HillClimbingv2MetadataPath = Path.Combine("ml", "models", "chess_move_matrix_cnn", "metadata.json");
        private static readonly InferenceSession HillClimbingv2Session = CreateHillClimbingv2Session();
        private static readonly Lazy<List<String>> HillClimbingv2Labels = new Lazy<List<String>>(LoadHillClimbingv2Labels);

        private Int32 HillClimbingv2Search(Position position, Int32 depth, Int32 alpha, Int32 beta, Int32 ply, Boolean inCheck, Boolean allowNull) {
            if (position == null)
                return Move.Invalid;
            ...
            for (Int32 currentDepth = 1; currentDepth <= maxDepth; currentDepth++) {
                ...
                List<Int32> candidateMoves = GetHillClimbingv2Candidates(position, rootMoves, HillClimbingv2TopN);
                ...
                if (iterationBestMove != Move.Invalid) {
                    ...
                    WriteHillClimbingLog(position, currentDepth, bestValue, GetHillClimbingPrincipalVariation(position, iterationBestMove, currentDepth));
                }
            }
            ...
            WriteHillClimbingv2Statistics(position, _stopwatch.Elapsed.TotalMilliseconds);
        ...
        private Int32 SearchNodev2(Position position, Int32 depth, Int32 ply, Int32 alpha, Int32 beta, Boolean inCheck) {
            ...
            List<Int32> moves = GetHillClimbingv2Candidates(position, position.LegalMoves(), HillClimbingv2TopN);
            if (moves.Count == 0)
                moves = position.LegalMoves();

            if (CanApplyNullMoveV2(position, inCheck, depth)) {
                position.MakeNull();
                Int32 nullValue = -SearchNodev2(position, depth - 1 - HillClimbingv2NullMoveReduction, ply + 1, -beta, -beta + 1, false);
                position.UnmakeNull();
                if (nullValue >= beta)
                    return nullValue;
            }
            ...
            for (Int32 i = 0; i < moves.Count; i++) {
                ...
                Boolean reducible = i >= HillClimbingv2LateMoveReductionMove && depth >= HillClimbingv2LateMoveReductionDepth && !tactical;
        ...
        private List<Int32> GetHillClimbingv2Candidates(Position position, List<Int32> legalMoves, Int32 topN) {
            ...
            foreach (var prediction in InvokeHillClimbingv2Model(position, legalMoves.Count)) {
        ...
        private static InferenceSession CreateHillClimbingv2Session() {
            try {
                String modelPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, HillClimbingv2ModelPath);
        ...
        private IEnumerable<(String label, Double score)> InvokeHillClimbingv2Model(Position position, Int32 topN) {
            List<(String label, Double score)> results = new List<(String label, Double score)>();
            if (HillClimbingv2Session == null || position == null)
                return results;

            try {
                Single[] inputData = BuildHillClimbingv2Input(position);
                DenseTensor<Single> input = new DenseTensor<Single>(inputData, new[] { 1, 18, 8, 8 });
                using (IDisposableReadOnlyCollection<DisposableNamedOnnxValue> output = HillClimbingv2Session.Run(new[] { NamedOnnxValue.CreateFromTensor("input", input) })) {
                    ...
                    for (int i = 0; i < take; i++) {
                        ...
                        if (bestIndex < 0)
                            break;
                        String label = bestIndex >= 0 && bestIndex < HillClimbingv2Labels.Value.Count ? HillClimbingv2Labels.Value[bestIndex] : String.Empty;
        ...
        private static float[] BuildHillClimbingv2Input(Position position) {
        ...
        private static List<String> LoadHillClimbingv2Labels() {
            try {
                String metadataPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, HillClimbingv2MetadataPath);
        ...
        private void WriteHillClimbingv2Statistics(Position position, Double elapsed) {
            if (Restrictions.Output != OutputType.GUI)
                return;
            ...
            Terminal.WriteLine(position.ToString(
                String.Format("HillClimbingv2 {0} ({1}-bit)", Version, IntPtr.Size * 8),
